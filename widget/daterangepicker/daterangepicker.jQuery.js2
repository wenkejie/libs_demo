...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			...
			{text: '年初至今', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today' },
			//extras:
			{text: '前一个月', dateStart: function(){ return Date.parse('1 month ago').moveToFirstDayOfMonth();  }, dateEnd: function(){ return Date.parse('1 month ago').moveToLastDayOfMonth();  } }
	...
	var datepickerOptions = {
		onSelect: function() { 
				...
				//2014-4-6  史磊： 选择开始日期时控制结束日历的最小选择日期。 选择结束日期时控制开始日历的最大选择日期。
				/*if($(this).is(".range-start")){
					rp.find('.range-end').datepicker("option","minDate",rangeA);
				}else{
					rp.find('.range-start').datepicker("option","maxDate",rangeB);
				}*/
			},
			defaultDate: +0,
			//2014-4-6  史磊：增加范围日历的选择范围控制。需要增加一个.ui-state-disabled 控制不能选择的效果。 
	...
	inputDateA = inputDateB = Date.parse('today');
	...
	var rpPresets = (function(){
		...
		jQuery.each(options.presets, function(key, value) {
			jQuery('<li class="ui-daterangepicker-'+ key +' preset_'+ x +' ui-helper-clearfix ui-corner-all"><span class="fonticon">&#x359e;</span><a href="#">'+ value +'</a></li>')
			.appendTo(ul);
			x++;
		});
		...
		ul.find('li').hover(
			...
			.click(function(){
				...
				var n = $('li', ul).index(this);
				
				
				if($(this).is('.ui-daterangepicker-dateRange')){
					var dw = document.documentElement.clientWidth,
						thisw = 680;
					if(thisw + options.posX > dw){
						rp.parent().animate({left: dw - thisw -20},300);
					}
				}else{
					setTimeout(function(){rp.parent().css("left",options.posX)},400);
				}
	...
	function showRP(){
		if(rp.data('state') == 'closed'){ 
			rp.data('state', 'open');
			//当元素在不可见时绑定该控件时，获取的初始坐标是错误的，改为在每次显示的时候重新获取坐标
			options.posX = rangeInput.offset().left; // x position
			options.posY = rangeInput.offset().top + rangeInput.outerHeight(); // y position
			if(rp.find('li.ui-state-active').is('.ui-daterangepicker-dateRange')){
					var dw = document.documentElement.clientWidth,
						thisw = 680;
					if(thisw + options.posX > dw){
						rp.parent().css("left", dw - thisw -20);
					}
			}else{
	...
	//picker divs
	var rpPickers = jQuery('<div class="ranges ui-widget-header ui-corner-all ui-helper-clearfix"><div class="range-start"><span class="title-start">开始日期</span></div><div class="range-end"><span class="title-end">End Date</span></div></div>').appendTo(rp);
 ...
 */
Date.CultureInfo={name:"en-US",englishName:"English (United States)",nativeName:"English (United States)",dayNames:["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],abbreviatedDayNames:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],shortestDayNames:["Su","Mo","Tu","We","Th","Fr","Sa"],firstLetterDayNames:["S","M","T","W","T","F","S"],monthNames:["January","February","March","April","May","June","July","August","September","October","November","December"],abbreviatedMonthNames:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],amDesignator:"AM",pmDesignator:"PM",firstDayOfWeek:0,twoDigitYearMax:2029,dateElementOrder:"mdy",formatPatterns:{shortDate:"yyyy-MM-dd",longDate:"dddd, MMMM dd, yyyy",shortTime:"h:mm tt",longTime:"h:mm:ss tt",fullDateTime:"dddd, MMMM dd, yyyy h:mm:ss tt",sortableDateTime:"yyyy-MM-ddTHH:mm:ss",universalSortableDateTime:"yyyy-MM-dd HH:mm:ssZ",rfc1123:"ddd, dd MMM yyyy HH:mm:ss GMT",monthDay:"MMMM dd",yearMonth:"MMMM, yyyy"},regexPatterns:{jan:/^jan(uary)?/i,feb:/^feb(ruary)?/i,mar:/^mar(ch)?/i,apr:/^apr(il)?/i,may:/^may/i,jun:/^jun(e)?/i,jul:/^jul(y)?/i,aug:/^aug(ust)?/i,sep:/^sep(t(ember)?)?/i,oct:/^oct(ober)?/i,nov:/^nov(ember)?/i,dec:/^dec(ember)?/i,sun:/^su(n(day)?)?/i,mon:/^mo(n(day)?)?/i,tue:/^tu(e(s(day)?)?)?/i,wed:/^we(d(nesday)?)?/i,thu:/^th(u(r(s(day)?)?)?)?/i,fri:/^fr(i(day)?)?/i,sat:/^sa(t(urday)?)?/i,future:/^next/i,past:/^last|past|prev(ious)?/i,add:/^(\+|after|from)/i,subtract:/^(\-|before|ago)/i,yesterday:/^yesterday/i,today:/^t(oday)?/i,tomorrow:/^tomorrow/i,now:/^n(ow)?/i,millisecond:/^ms|milli(second)?s?/i,second:/^sec(ond)?s?/i,minute:/^min(ute)?s?/i,hour:/^h(ou)?rs?/i,week:/^w(ee)?k/i,month:/^m(o(nth)?s?)?/i,day:/^d(ays?)?/i,year:/^y((ea)?rs?)?/i,shortMeridian:/^(a|p)/i,longMeridian:/^(a\.?m?\.?|p\.?m?\.?)/i,timezone:/^((e(s|d)t|c(s|d)t|m(s|d)t|p(s|d)t)|((gmt)?\s*(\+|\-)\s*\d\d\d\d?)|gmt)/i,ordinalSuffix:/^\s*(st|nd|rd|th)/i,timeContext:/^\s*(\:|a|p)/i},abbreviatedTimeZoneStandard:{GMT:"-000",EST:"-0400",CST:"-0500",MST:"-0600",PST:"-0700"},abbreviatedTimeZoneDST:{GMT:"-000",EDT:"-0500",CDT:"-0600",MDT:"-0700",PDT:"-0800"}};
